...
class GammaLayer(nn.Module):
    def __init__(self, num_attention_heads):
        ...
        self.gammas = nn.Parameter(torch.zeros(num_attention_heads, 1, 1))
        
        
        
        
    def forward(self, dist_scores):
        softplus = nn.Softplus()
        gamma = -1.0 * softplus(self.gammas).unsqueeze(0)
...
class MonotonicConvolutionalMultiheadAttention(nn.Module):
    ...
    def __init__(self, hidden_size, n_splits, dropout_p, head_ratio=2, conv_kernel_size=9):
        ...
        self.gamma_layer = GammaLayer(n_splits // head_ratio)
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        conv_kernel_layer = self.conv_kernel_layer(conv_attn_layer) # the extra linear layer initialized
       ...
       # m = nn.Softplus()
        ...
        total_effect = self.gamma_layer(dist_scores)
